...
Edit ThingWorx service code (JavaScript and SQL) as plain local files in VS Code, then sync it back and forth with a ThingWorx server.

TWLS turns a workspace folder into a working copy of a ThingWorx **entity** — a Thing Shape or Thing Template. Its services are pulled down into real `.js` / `.sql` files where you can use your normal editor tooling, see exactly what changed in the Source Control view, and push those changes back to ThingWorx.

## Features

- **Two-way sync with ThingWorx** — pull services down to disk, edit locally, push them back up to the server.
- **Source Control integration** — modified and deleted services appear as changes in a dedicated *TWLS* panel, complete with an inline diff against the remote version.
- **File-level granularity** — each service maps to a single `.js` (script) or `.sql` (query) file, and each file is tracked independently.
...
2. Services of that entity are written to disk under a path that mirrors ThingWorx's project structure:
...
TWLS writes the `.twls/thingworx.json` config, pulls the entity's services into the folder, and they're ready to edit.
...
| `TWLS: Pull` | Download the active entity's services, overwriting local files. No-op when nothing changed on the server. |
| `TWLS: Pull Project` | Download the services of every entity in a project you pick. |
| `TWLS: Push` | Upload all changed local service files to ThingWorx. |
| `TWLS: Switch Entity` | Point the folder at a different entity and pull its services in. |
| `TWLS: Discard` | Revert a single changed file back to its remote version. |

### Working with changes

- Open the **Source Control** view and select the **TWLS** repository to see which services changed. Modified and deleted files are listed with appropriate icons.
